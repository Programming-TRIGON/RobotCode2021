...
public class SwerveModule {
	 ...
	 */
	public SwerveModule(int speedID, int angleID, double wheelDiameter) {
		this.wheelDiameter = wheelDiameter;
		speedMotor = new TrigonTalonFX(speedID, RobotConstants.staticConstants.SWERVE_MODULE_SPEED_MOTOR_CONFIG);
		speedController = new TrigonPIDController(RobotConstants.staticConstants.SWERVE_MODULE_SPEED_PID_COEFS);

		angleMotor = new TrigonTalonFX(angleID, RobotConstants.staticConstants.SWERVE_MODULE_ANGLE_MOTOR_CONFIG);
		angleController = new TrigonPIDController(RobotConstants.staticConstants.SWERVE_MODULE_ANGLE_PID_COEFS);
		angleMotor.configSelectedFeedbackSensor(RobotConstants.staticConstants.SWERVE_MODULE_ANGLE_MOTOR_FEEDBACK_DEVICE);
	...
	public void periodic() {
		speedMotor.set(speedController.calculate(getMps()));
	...
	public double getMps() {
		return getSpeed() / 10 / RobotConstants.staticConstants.TALON_FX_TICKS_PER_REVOLUTION * wheelDiameter;
	}
	...
	public double getAngle() {
		return angleMotor.getSelectedSensorPosition() / RobotConstants.staticConstants.TALON_FX_TICKS_PER_REVOLUTION * 360;
	}
}
